 ...
 */
export const getWorkshopsForUserVehicles = async (userVehicles) => {
  try {
    ...
    for (const vehicle of userVehicles) {
      if (vehicle.id) {
        try {
          ...
          console.error(`Error obteniendo talleres para vehículo ${vehicle.id}:`, error);
 ...
 */
export const getMechanicsForUserVehicles = async (userVehicles) => {
  try {
    if (!userVehicles || userVehicles.length === 0) {
      return [];
    }
